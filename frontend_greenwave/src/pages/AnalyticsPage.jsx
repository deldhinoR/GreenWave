    ...
    <div className="page-stack" style={{ gap: "1rem" }}>
      <div className="page-frame-header">
        ...
        <div className="section-heading compact">
          <span className="eyebrow">{tr ? "Sıkışıklık Analitiği" : "Congestion Analytics"}</span>
      ...
      <section className="hero-row">
        <div className="section-heading">
          <span className="eyebrow live">{tr ? "Ortalama Sıkışıklık Geçmişi" : "Average Congestion History"}</span>
          <h2>{range === "24h" ? (tr ? "Son 24 Saat (Saatlik)" : "Last 24 Hours (Hourly)") : range === "7d" ? (tr ? "Son 7 Gün (Günlük)" : "Last 7 Days (Daily)") : (tr ? "Son 30 Gün (Günlük)" : "Last 30 Days (Daily)")}</h2>
        </div>

        <div className="button-group">
          <button className={`secondary-button ${range === "24h" ? "active-tab" : ""}`} type="button" onClick={() => setRange("24h")}>{tr ? "Son 24 Saat" : "Last 24 Hours"}</button>
          <button className={`secondary-button ${range === "7d" ? "active-tab" : ""}`} type="button" onClick={() => setRange("7d")}>{tr ? "Son 7 Gün" : "Last 7 Days"}</button>
          <button className={`secondary-button ${range === "30d" ? "active-tab" : ""}`} type="button" onClick={() => setRange("30d")}>{tr ? "Son 30 Gün" : "Last 30 Days"}</button>
        </div>
      </section>
