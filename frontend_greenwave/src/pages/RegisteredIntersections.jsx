  ...
  return (
    <div className="page-content">
      <div className="page-header">
        <h1 className="page-title">{tr ? "Kayıtlı Bölgeler" : "Registered Regions"}</h1>
        <p className="page-subtitle">{tr ? "SUMO proje çıktılarından kaydedilen tüm kavşak bölgeleri" : "All saved intersection regions from SUMO project exports"}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <h3>{tr ? "Bölge Bulunamadı" : "No Regions Found"}</h3>
          <p>{tr ? "Henüz kayıtlı bölge yok. Bir bölge kaydetmek için Editör'den SUMO projesi oluşturun." : "No saved regions yet. Create a SUMO project from the Editor to register a region."}</p>
        </div>
      ) : (
        <div className="table-wrapper">
          <table className="data-table">
            <thead>
              <tr>
                <th>ID</th>
                <th>{tr ? "Bölge Adı (Önbellek Anahtarı)" : "Region Name (Cache Key)"}</th>
                <th>{tr ? "Oluşturulma" : "Created At"}</th>
                <th>{tr ? "Konum Sınırları" : "Location Bounds"}</th>
                <th>{tr ? "İşlemler" : "Actions"}</th>
              </tr>
            </thead>
            <tbody>
              {regions.map((region) => (
                <tr key={region.id}>
                  ...
                  <td className="actions-cell">
                    <button 
                      ...
                      title={tr ? "Bölgeyi Sil" : "Delete Region"}
                    >
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <h3>{tr ? "Bölgeyi Sil" : "Delete Region"}</h3>
              <button 
            ...
            <div className="modal-body">
              <p>{tr ? <>Bölgeyi silmek istediğinize emin misiniz <strong>"{deleteConfirm.name}"</strong>?</> : <>Are you sure you want to delete the region <strong>"{deleteConfirm.name}"</strong>?</>}</p>
              <p className="warning-text">{tr ? "Bu işlem geri alınamaz." : "This action cannot be undone."}</p>
            </div>
