...
app.config["SECRET_KEY"] = os.environ.get("GREENWAVE_SECRET_KEY", "greenwave-dev-secret")
TOKEN_SERIALIZER = URLSafeTimedSerializer(app.config["SECRET_KEY"], salt="auth")
AUTH_COOKIE_NAME = "gw_session"
create_table()


def _issue_auth_token(user_id, username, role):
    return TOKEN_SERIALIZER.dumps({"user_id": int(user_id), "username": username, "role": role})


def _decode_auth_token(token, max_age=60 * 60 * 24 * 30):
    if not token:
        return None
    try:
...
def auth_login():
    ...
    if role != "admin":
        return jsonify({"error": "Only admin accounts can access this system"}), 403
    if not check_password_hash(password_hash, password):
        return jsonify({"error": "Invalid credentials"}), 401

    token = _issue_auth_token(user_id, uname, role)
    response = jsonify({"ok": True, "user": {"id": user_id, "username": uname, "role": role}})
    response.set_cookie(
        AUTH_COOKIE_NAME,
        token,
        httponly=True,
...
def auth_change_password():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT password_hash FROM users WHERE id = %s", (int(user["user_id"]),))
        row = cursor.fetchone()
        ...
        cursor.execute(
            ...
            WHERE id = %s
            """,
            (generate_password_hash(new_password), int(user["user_id"])),
        )
        conn.commit()
    finally:
        conn.close()
...
def get_my_settings():
    ...
    try:
        cursor.execute(
            """
            SELECT u.username, COALESCE(s.theme, 'light'), COALESCE(s.language, 'English (US)')
            FROM users u
    ...
    return jsonify({"username": row[0], "theme": row[1], "language": row[2]}), 200


@app.route("/settings/me", methods=["PUT"])
@require_admin
def update_my_settings():
    user = _get_current_user()
    payload = request.get_json(silent=True) or {}
    theme = str(payload.get("theme", "light")).strip().lower()
    language = str(payload.get("language", "English (US)")).strip() or "English (US)"
    ...
    try:
        cursor.execute(
            ...
            ON CONFLICT (user_id) DO UPDATE SET
                ...
                updated_at = NOW()
            """,
            (int(user["user_id"]), theme, language),
        )
        conn.commit()
    finally:
        conn.close()
    return jsonify({"ok": True, "theme": theme, "language": language}), 200
...
def _runtime_improvement_from_sim_logs(experiment_id, span=120, window=20):
    """
    Compute congestion improvement from simulation_logs only (vision-independent).
    We compare average congestion score in an early window vs latest window.
    """
    if span < 10:
        span = 10
    if window < 5:
    ...
    try:
        with get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT queue_length, average_waiting_time, density_level
                    FROM simulation_logs
                    WHERE experiment_id = %s
                    ORDER BY step DESC
                    LIMIT %s
                    """,
                    (int(experiment_id), int(span)),
                )
                rows = cursor.fetchall()
    except Exception:
        return None

    if not rows or len(rows) < (window * 2):
        return None
    ...
    baseline_score = _avg_score(early)
    current_score = _avg_score(late)

    if baseline_score <= 0:
        return None

    improvement = ((baseline_score - current_score) / baseline_score) * 100.0
...
def intersection_live_dashboard(intersection_id):
    ...
    try:
        with get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    """
                    SELECT selected_phase, green_duration, red_duration
                    FROM decision_logs
                    WHERE experiment_id = %s
                    ORDER BY step DESC
                    LIMIT 8
    ...
    if avg_congestion is None:
        avg_congestion = 0.0
    # Use finalized KPI only (written at simulation end) so the card value stays stable.
        ...
        with get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                if kpi_row is not None and kpi_row[0] is not None:
                    congestion_improvement = max(-100.0, min(100.0, float(kpi_row[0])))
    except Exception:
        congestion_improvement = None
    ...
    return jsonify(
        {
            ...
            "cards": {
                "liveCongestionImprovement": 23.3,
    ...
    try:
        with get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    """
                    SELECT step, selected_phase, green_duration, red_duration, created_at
                    FROM decision_logs
                    WHERE experiment_id = %s
                    ORDER BY step DESC
                    LIMIT %s
    ...
    return jsonify(
        {
            ...
            "latest": {
                ...
                "redSeconds": int(latest_dec[3] or 0) if latest_dec else None,
            },
            "recentSimulation": [
                {
                    "step": int(row[0] or 0),
