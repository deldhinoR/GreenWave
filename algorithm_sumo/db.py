...
_DB_ERROR_PRINTED = False

def get_connection():
    global _DB_UNAVAILABLE, _DB_ERROR_PRINTED
    if _DB_UNAVAILABLE:
        return None
    try:
        ...
        if not _DB_ERROR_PRINTED:
            print("Database connection error:", e)
            print("DB logging disabled for this run. Start PostgreSQL to enable logging.")
            _DB_ERROR_PRINTED = True
...
def init_db():
    ...
    # Runtime KPI summary per experiment (written by algorithm_sumo)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS experiment_runtime_kpis (
            id SERIAL PRIMARY KEY,
            experiment_id INTEGER NOT NULL,
            congestion_improvement DOUBLE PRECISION NOT NULL,
            baseline_wait DOUBLE PRECISION NOT NULL,
            current_wait DOUBLE PRECISION NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
    """)
...
def create_experiment(preferred_id=None):
    conn = get_connection()
    if conn is None:
...
def log_experiment_runtime_kpi(
    experiment_id,
    ...
    cur.execute(
        """
        INSERT INTO experiment_runtime_kpis
        (
            experiment_id,
            congestion_improvement,
            baseline_wait,
