  ...
  useEffect(() => {
    window.__gwToggleRegionSystem = async (regionId) => {
      ...
      try {
        ...
        alert("Failed to update system state. Check backend logs.");
      }
    };
    return () => {
      ...
      const popupContent = `
        <div style="min-width: 200px; font-size: 13px;">
          ...
          <div style="color: #556377; line-height: 1.6;">
            ...
            <div><strong>Created:</strong> ${formatDate(region.createdAt)}</div>
            ...
            <div style="margin-top: 14px; text-align: center;">
              <button type="button" class="popup-details-btn" onclick="window.location.href='/region/${region.id}/dashboard'">See stats</button>
              <button type="button" class="popup-edit-btn" onclick="window.location.href='/editor?regionId=${region.id}&min_lat=${region.min_lat}&min_lon=${region.min_lon}&max_lat=${region.max_lat}&max_lon=${region.max_lon}&regionName=${encodeURIComponent(region.name || "")}'">Edit system</button>
              <button type="button" class="${isRegionRunning(region.id) ? "popup-stop-btn" : "popup-start-btn"}" onclick="window.__gwToggleRegionSystem(${region.id})">${isRegionRunning(region.id) ? "Stop" : "Start"}</button>
            </div>
          </div>
      `;
    ...
    <div className="overview-container">
      <div className="overview-header">
        <div className="header-intro">
          <h1 className="page-title">Region Overview</h1>
          <p className="page-subtitle">Click a region marker and choose See stats or Edit system.</p>
          <div className="system-actions">
            <span className="system-status-text">
              Active Region: {systemStatus?.active_region_id || "-"} | API: {systemStatus?.greenwave_api?.running ? "ON" : "OFF"} | Camera: {systemStatus?.camera_detection?.running ? "ON" : "OFF"} | SUMO: {systemStatus?.algorithm_sumo?.running ? "ON" : "OFF"}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="regions-list-section">
        ...
        {regions.length === 0 ? (
          <div className="regions-empty">No regions found.</div>
        ) : (
            ...
            {regions.map((region) => (
              <div className="region-row" key={region.id}>
                <div className="region-main">
                  ...
                  <span>{region.address || "No address"}</span>
                </div>
                <div className="region-actions">
                  ...
                  >
                    {isRegionRunning(region.id) ? "Stop" : "Start"}
                  </button>
                </div>
              </div>
            ))}
