...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Aggregate camera dataset JSON files into hourly congestion averages."
    )
    parser.add_argument(
        "--dataset-root",
...
def aggregate(dataset_root: Path):
    ...
    for json_file in iter_json_files(dataset_root):
        ...
        key = (parsed["junction_id"], parsed["hour_ts"])
        bucket = grouped[key]
...
def upsert_hourly(conn, grouped):
    cur = conn.cursor()
    rows_written = 0

    for (junction_id, hour_ts), bucket in grouped.items():
        ...
        cur.execute(
            ...
            INSERT INTO camera_congestion_hourly
                (junction_id, hour_ts, avg_congestion, min_congestion, max_congestion, sample_count, road_sample_count)
            VALUES (?, ?, ?, ?, ?, ?, ?)
...
def main():
    ...
    rows_written = upsert_hourly(conn, grouped)

    print(f"Dataset root: {dataset_root}")
    print(f"DB path: {db_path}")
    print(f"JSON files scanned: {file_count}")
    print(f"Valid records: {valid_count}")
    print(f"Hourly aggregate rows upserted: {rows_written}")
